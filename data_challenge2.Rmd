...
**WARNING: This process is optional and it will take a significant amount of time.**
...
with.clu
```
> it seems that the more clusters we have, the less within-cluster variation we have (which is obvious). 

## Analyzing Clusters (example of 4-cluster model)
```{r}
clu <- 3
...
sort(centroid.i - m.centroid.noti, decreasing = T)[1:10]
```
> Normalized data is not very useful when applying k-means clustering to this type of dataset because of the overly generalized words cancelling out other keywords that we might be interested in. 



## Bigram Analysis
### we are only looking at unnoramrlized data from now on
```{r}
